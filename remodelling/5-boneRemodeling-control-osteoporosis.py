...
def myControl(intON, intOFF, bisPAR, TG1PAR, winPAR):
    # MODEL
    def rhsOFF(x,t):
        # MODEL PARAMETERS
        aC = p0 
        ...
        xCdot = aC*xT/xB - (bC+bis)*xC - bCT*xT*xC
        xBdot = aBW*xB*xW/xT - bB*xB
        xTdot = aT*kC*xC*BM - bT*xT - TG1*xT
        xWdot = aW*xC - bW*xW + win
        BMdot = kB*xB - kC*xC*BM
        
        fx = np.array([xCdot, xBdot, xTdot, xWdot, BMdot])
        return fx
    ...
    x0 = np.array([xC0, xB0, xT0, xW0, BM0])
    ...
    while tfAux < tf:
        ...
        if i%2==0:
            ...
            controlAux = np.array([[bisAux],[TG1Aux],[winAux]])
        if i%2==1:
        ...
        if i==0:
            ...
            TG1Final = TG1Aux
            winFinal = winAux
            myControlFinal = np.array([[bisFinal],[TG1Final],[winFinal]])
        else:
...
def extract_ctrl(folder):
    ...
    return x, bis, tgf, wnt, xC, xB, xT, xW, BM
...
def plotResults(state,control,time):
    ...
    # no control
    NOstate, NOcontrol, NOtime = myControl(1, 1, 0, 0, 0)
...
state, control, time = myControl(249, 1, 0.05, 1, 1)
plotResults(state, control, time)
plt.savefig("control-rem-inter-1-1.pdf")

state, control, time = myControl(2, 19, 0.05, 1, 1)
plotResults(state, control, time)
plt.savefig("control-rem-inter-1-2.pdf")
#
x, bis, tgf, wnt, xC, xB, xT, xW, BM = extract_ctrl('rem-sol1')
optimalState = np.transpose(np.array([[xC],[xB],[xT],[xW],[BM]]))
optimalControl = np.array([[bis],[tgf],[wnt]])
optimalTime = np.array(x)
plotResults(optimalState[:,0,:], optimalControl, optimalTime)
plt.savefig("control-rem-optimal-1.pdf")


state, control, time = myControl(249, 1, 0.05, 5, 5)
plotResults(state, control, time)
plt.savefig("control-rem-inter-2-1.pdf")

state, control, time = myControl(2, 19, 0.05, 5, 5)
plotResults(state, control, time)
plt.savefig("control-rem-inter-2-2.pdf")

x, bis, tgf, wnt, xC, xB, xT, xW, BM = extract_ctrl('rem-sol2')
optimalState = np.transpose(np.array([[xC],[xB],[xT],[xW],[BM]]))
optimalControl = np.array([[bis],[tgf],[wnt]])
optimalTime = np.array(x)
plotResults(optimalState[:,0,:], optimalControl, optimalTime)
plt.savefig("control-rem-optimal-2.pdf")
...
ax = plt.subplot(111)
my_soln, my_control, time = myControl(2, 2, 0.0, 0.0, 0)
plt.plot(time, my_soln[:,4],color='black', linestyle='dashed', label='no control')#
my_soln, my_control, time = myControl(2, 19, 0.0, 0.0, 1)
plt.plot(time, my_soln[:,4], label=r'$u_{wn1}=1$')
my_soln, my_control, time = myControl(2, 19, 0.0, 0.0, 5)
plt.plot(time, my_soln[:,4], label=r'$u_{wn1}=5$')
my_soln, my_control, time = myControl(2, 19, 0.0, 0.0, 10)
plt.plot(time, my_soln[:,4], label=r'$u_{wn1}=10$')
plt.ylim([0.4,1.2])
plt.xlabel('Time (days)',fontsize=12)
plt.ylabel('Bone Mass',fontsize=12)
plt.legend()
...
plt.savefig("control-rem-wnt.pdf")
...
my_soln, my_control, time = myControl(2, 2, 0.0, 0.0, 0)
plt.plot(time, my_soln[:,4],color='black', linestyle='dashed', label='no control')#
my_soln, my_control, time = myControl(2, 19, 0.05, 0.0, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{bis}=0.05$')
my_soln, my_control, time = myControl(2, 19, 0.2, 0.0, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{bis}=0.2$')
my_soln, my_control, time = myControl(2, 19, 0.4, 0.0, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{bis}=0.4$')
plt.ylim([0.4,1.2])
plt.xlabel('Time (days)',fontsize=12)
plt.ylabel('Bone Mass',fontsize=12)
plt.legend()
...
ax.set_position([box.x0, box.y0, box.width * 0.65, box.height])
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.savefig("control-rem-bis.pdf")
...
my_soln, my_control, time = myControl(2, 2, 0.0, 0.0, 0)
plt.plot(time, my_soln[:,4],color='black', linestyle='dashed', label='no control')#
my_soln, my_control, time = myControl(2, 19, 0.0, 5, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{tg1}=5$')
my_soln, my_control, time = myControl(2, 19, 0.0, 20, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{tg1}=20$')
my_soln, my_control, time = myControl(2, 19, 0.0, 50, 0)
plt.plot(time, my_soln[:,4], label=r'$u_{tg1}=50$')
plt.ylim([0.4,1.2])
plt.xlabel('Time (days)',fontsize=12)
plt.ylabel('Bone Mass',fontsize=12)
plt.legend()
...
ax.set_position([box.x0, box.y0, box.width * 0.65, box.height])
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
plt.savefig("control-rem-tg1.pdf")
'''
